...
async function refreshFx(asOf) {
  ...
  const previous = await readJson(`https://api.frankfurter.dev/v1/${previousWeekdayUtc()}?from=SGD`).catch(
    () => null,
  )
  return writeIfChanged('fx.json', {
    asOf,
    source:
      'open.er-api.com/v6/latest/SGD + api.frankfurter.dev previous weekday (refreshed on the published site by tools/refresh-snapshot.mjs)',
    latest,
